import React, { useState } from 'react';
import { useFormik } from 'formik';
import { FaEyeSlash, FaEye } from 'react-icons/fa';
import Logo from "../../assets/logo1.png";

const userData = {
  name: "Daniel",
  phone: "777-233-33-33",
  lastName: "Arellano",
  email: "daniel@gmail.com",
};

const validate = (formValues) => {
  const errors = {};

  if (formValues.email !== userData.email && formValues.email !== '') {
    if (!formValues.email) {
      errors.email = 'El correo electrónico es requerido';
    } else if (!/\S+@\S+\.\S+/.test(formValues.email)) {
      errors.email = 'El correo electrónico es inválido';
    }
  }

  if (formValues.phone !== userData.phone && formValues.phone !== '') {
    if (!formValues.phone) {
      errors.phone = 'El teléfono es requerido';
    } else if (!/^\d{3}-\d{3}-\d{2}-\d{2}$/.test(formValues.phone)) {
      errors.phone = 'El teléfono debe tener el formato 777-000-00-00';
    }
  }

  if (formValues.name !== userData.name && formValues.name !== '') {
    if (!formValues.name) {
      errors.name = 'El nombre es requerido';
    }
  }

  if (formValues.lastName !== userData.lastName && formValues.lastName !== '') {
    if (!formValues.lastName) {
      errors.lastName = 'El apellido es requerido';
    }
  }

  if (!formValues.newPassword) {
    errors.newPassword = 'Ingrese una nueva contraseña';
  }

  if (!formValues.confirmPassword) {
    errors.confirmPassword = 'Confirme la nueva contraseña';
  } else if (formValues.newPassword !== formValues.confirmPassword) {
    errors.confirmPassword = 'Las contraseñas no coinciden';
  }

  if (!formValues.currentPassword) {
    errors.currentPassword = 'Ingrese su contraseña actual';
  }

  return errors;
};

function Perfil() {
  const [showNewPassword, setShowNewPassword] = useState(false);
  const [showConfirmPassword, setShowConfirmPassword] = useState(false);
  const [showCurrentPassword, setShowCurrentPassword] = useState(false);
  const [formValues, setFormValues] = useState({
    email: userData.email,
    phone: userData.phone,
    name: userData.name,
    lastName: userData.lastName,
  });
  const formik = useFormik({
    initialValues: {
      newPassword: '',
      confirmPassword: '',
      currentPassword: '',
    },
    validate,
    onSubmit: (values) => {
      // Lógica para guardar la nueva contraseña
      console.log(values);
    },
  });

  const formikHandler = useFormik({
    initialValues: {
      email: '',
      phone: '',
      name: '',
      lastName: '',
    },
    validate: (values) => validate(formValues),
    onSubmit: (values) => {
      // Lógica para enviar los datos
      console.log(formValues);
    },
  });



  return (
    <div className='h-screen '>
      <div className='grid  mb-5 justify-items-center '>
        <img width="20%" height="20%" src={Logo} alt="Logo de la carniceria" />
        <p className='text-3xl font-extrabold text-center '> {userData.name} {userData.lastName}</p>
        <div className='grid grid-flow-col gap-4'>
          <p className='text-xl font-extrabold text-center'>{userData.email}</p>
          <p className='text-xl font-extrabold text-center'> {userData.phone}</p>
        </div>
      </div>
      <div className='w-full bg-red p-2 rounded-t-lg'>
        <p className='text-white text-light-600'>Actualizar Información</p>
      </div>
      <form onSubmit={formikHandler.handleSubmit}>
        <div className='grid grid-cols-2 gap-4 px-5 pt-5'>
          <div>
            <label htmlFor="email" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Correo Electrónico</label>
            <input
              type="text"
              id="email"
              name="email"
              className="effect-shadow-input w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
              placeholder=""
              required
              value={formValues.email}
              onChange={(e) => setFormValues({ ...formValues, email: e.target.value })}
            />
            {formikHandler.errors.email ? <div className="text-red-500">{formikHandler.errors.email}</div> : null}

          </div>
          <div>
            <label htmlFor="phone" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Teléfono</label>
            <input
              type="text"
              id="phone"
              name="phone"
              className="effect-shadow-input w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
              placeholder=""
              required
              value={formValues.phone}
              onChange={(e) => setFormValues({ ...formValues, phone: e.target.value })}
            />
            {formikHandler.errors.phone ? <div className="text-red-500">{formikHandler.errors.phone}</div> : null}
          </div>
        </div>
        <div className='grid grid-cols-2 gap-4 px-5 pt-5'>
          <div>
            <label htmlFor="name" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nombre</label>
            <input
              type="text"
              id="name"
              name="name"
              className="effect-shadow-input w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
              placeholder=""
              required
              value={formValues.name}
              onChange={(e) => setFormValues({ ...formValues, name: e.target.value })}
            />
            {formikHandler.errors.name ? <div className="text-red-500">{formikHandler.errors.name}</div> : null}
          </div>
          <div>
            <label htmlFor="lastName" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Apellidos</label>
            <input
              type="text"
              id="lastName"
              name="lastName"
              className="effect-shadow-input w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
              placeholder=""
              required
              value={formValues.lastName}
              onChange={(e) => setFormValues({ ...formValues, lastName: e.target.value })}
            />
            {formikHandler.errors.lastName ? <div className="text-red-500">{formikHandler.errors.lastName}</div> : null}
          </div>
        </div>
        <div className="grid justify-end m-5">
          <button type="submit" className="text-red-700 hover:text-white border border-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red dark:focus:ring-red-900">Actualizar</button>
        </div>
      </form>


      <div>
        <div className="w-full bg-red p-2 rounded-t-lg">
          <p className="text-white text-light-600">Cambio contraseña</p>
        </div>
        <div className="">
          <form onSubmit={formik.handleSubmit}>
            <div className="grid grid-cols-3 gap-4 px-5 pt-2 pb-3">
              <div>
                <label htmlFor="newPassword" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                  Nueva Contraseña
                </label>
                <div className="relative">
                  <input
                    type={showNewPassword ? 'text' : 'password'}
                    id="newPassword"
                    className="effect-shadow-input w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    {...formik.getFieldProps('newPassword')}
                    required
                  />
                  <div className="absolute inset-y-0 right-0 pr-3 flex items-center">
                    <button
                      type="button"
                      onClick={() => setShowNewPassword(!showNewPassword)}
                      className="text-red-500 hover:text-red-700"
                    >
                      {showNewPassword ? <FaEyeSlash /> : <FaEye />}
                    </button>
                  </div>
                </div>
                {formik.errors.newPassword && <div className="text-red-500 ">{formik.errors.newPassword}</div>}
              </div>
              <div>
                <label htmlFor="confirmPassword" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                  Confirmar Contraseña
                </label>
                <div className="relative">
                  <input
                    type={showConfirmPassword ? 'text' : 'password'}
                    id="confirmPassword"
                    className="effect-shadow-input w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    {...formik.getFieldProps('confirmPassword')}
                    required
                  />
                  <div className="absolute inset-y-0 right-0 pr-3 flex items-center">
                    <button
                      type="button"
                      onClick={() => setShowConfirmPassword(!showConfirmPassword)}
                      className="text-red-500 hover:text-red-700"
                    >
                      {showConfirmPassword ? <FaEyeSlash /> : <FaEye />}
                    </button>
                  </div>
                </div>
                {formik.errors.confirmPassword && <div className="text-red-500">{formik.errors.confirmPassword}</div>}
              </div>
              <div>
                <label htmlFor="currentPassword" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                  Contraseña Actual
                </label>
                <div className="relative">
                  <input
                    type={showCurrentPassword ? 'text' : 'password'}
                    id="currentPassword"
                    className="effect-shadow-input w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"

                    {...formik.getFieldProps('currentPassword')}
                    required
                  />
                  <div className="absolute inset-y-0 right-0 pr-3 flex items-center">
                    <button
                      type="button"
                      onClick={() => setShowCurrentPassword(!showCurrentPassword)}
                      className="text-red-500 hover:text-red-700"
                    >
                      {showCurrentPassword ? <FaEyeSlash /> : <FaEye />}
                    </button>
                  </div>
                </div>
                {formik.errors.currentPassword && <div className="text-red-500">{formik.errors.currentPassword}</div>}
              </div>
            </div>
            <div className="grid justify-end m-5">
              <button
                type="submit"
                className="text-red-700 hover:text-white border border-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red dark:focus:ring-red-900"
              >
                Guardar
              </button>
            </div>
          </form>
        </div>
      </div>

    </div>
  )
}

export default Perfil